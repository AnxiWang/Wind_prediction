...
class DeepFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            if self.loss_type == "logloss":
                self.out = tf.nn.sigmoid(self.out, name="output1")
                self.loss = tf.losses.log_loss(self.label, self.out)
            elif self.loss_type == "mse":
                self.loss = tf.nn.l2_loss(tf.subtract(self.label, self.out))
            elif self.loss_type == "exp-loss":
                self.loss = tf.nn.exp(- self.label * self.out)
            ...
            # decay learning rate
            self.learning_rate = tf.train.exponential_decay(self.learning_rate, self.epoch, 5, 0.8, staircase=True)
    ...
    def _initialize_weights(self):
        ...
        weights["feature_embeddings"] = tf.Variable(
            ...
            tf.random_uniform([self.feature_size, self.embedding_size], minval=-0.1, maxval=0.1),
    ...
    def evaluate(self, Xi, Xv, y):
        ...
        # dist = np.sqrt(np.sum(np.square(y - y_pred)) / len(y))
        dist = np.sum(np.abs(y - y_pred) / y) / len(y)
        return dist
